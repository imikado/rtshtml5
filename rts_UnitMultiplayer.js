...
Unit.prototype={
	...
	animateBroadcast:function(action){
		socket.emit('unit.animateBroadcast',this.id,action);
	},
	...
	buildBroadcast:function(){
		socket.emit('unit.buildBroadcast',this.id,this.x,this.y);
		console.log('broadcast unit.build'+this.id+' '+this.x+' '+this.y);
	},
	build:function(){
	...
	buildOnBroadcast:function(oBuildOn){
		socket.emit('unit.buildOnBroadcast',this.id,oBuildOn.name,oBuildOn.x,oBuildOn.y);
	},
